...
type
  TBoldModelDM = class(TDataModule)
    ...
    IndividPersonsADOQueryBOLD_ID: TIntegerField;
    IndividPersonsADOQueryBOLD_TYPE: TSmallintField;
    IndividPersonsADOQueryFamiliya: TStringField;
    IndividPersonsADOQueryImya: TStringField;
    IndividPersonsADOQueryOtchestvo: TStringField;
    IndividPersonsADOQueryUlica: TStringField;
    IndividPersonsADOQueryDom: TIntegerField;
    IndividPersonsADOQueryKvartira: TIntegerField;
    IndividPersonsADOQueryKorpus: TIntegerField;
    IndividPersonsADOQueryMobiljnyi_telefon: TStringField;
    IndividPersonsADOQueryRabochii_telefon: TStringField;
    IndividPersonsADOQueryDomashnii_telefon: TStringField;
    IndividPersonsADOQueryElektronnyi_adres: TStringField;
    IndividPersonsADOQueryRESERVED_PRESENTS: TIntegerField;
    IndividPersonsADOQueryoption_id: TIntegerField;
    IndividPersonsADOQueryOPTION_LOOK: TStringField;
    AdrPhoneRelatingADOTableBOLD_ID: TIntegerField;
    AdrPhoneRelatingADOTableBOLD_TYPE: TSmallintField;
    AdrPhoneRelatingADOTableTelefon_klienta: TStringField;
    AdrPhoneRelatingADOTableAdres_vyzova_vvodim: TStringField;
    AdrPhoneRelatingADOTableotnositsya_k_sektoru: TIntegerField;
    AdrPhoneRelatingADOTableSummarn_chislo_vyzovov: TIntegerField;
    AdrPhoneRelatingADOTablebonus_summ: TBCDField;
    AdrPhoneRelatingADOTableoption_id: TIntegerField;
    AdrPhoneRelatingADOTableOPTION_LOOK: TStringField;
     ...
     if INumPourPhoneADOQuery.FieldByName('korpus').AsInteger>0  then
      res:=
       ...
       begin
         res:=
          IntToStr(INumPourPhoneADOQuery.FieldByName('korpus').AsInteger);
        //ShowMessage('Девушка, у вас несколько абонентских номеров с введенным вами номером телефона, проверьте справочник абонентских номеров!')
       end
    else
      begin
      end;
  except
    ShowMessage('Неудачное прохождение запроса на поиск абонента по телефонному номеру!');
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос наличия общепризовой заявки в базе по инд. номеру абонента!');
  end;
  end; 
  Result:=
     res;
    ...
    if HistoryINumADOQuery.RecordCount>0 then
     begin
      ...
      {try
        ...
        ShowMessage('Неудачный запрос объекта исходного сектора!');
      end;  }
     end;
  except
    ShowMessage('Неудачный запрос количества заявок по индивидуальному номеру абонента!');
  end;
 end;
Result:=res;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос нового персонального номера абонента!');
  end;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос абонента по его индивидуальному номеру!');
  end;
end;
...
begin
  ...
  try
   ...
   begin
    ...
    ShowMessage('Неудачный запрос наличия общепризовой заявки в базе!');
  end;
  end;
  Result:=
     res;
...
begin
FirstForm.StatusBar.Panels[2].Text:='ИЗМЕНЕНИЯ ОТОСЛАНЫ';
 if not (IniFile.ReadString('BUSINESS_DATA','enable_for_allXML_DEMO','NO')='YES') then
  if IniFile.ReadString('BUSINESS_DATA','check_refresh_by_ask','NO')='YES' then
   ...
   begin
    if not SendSQL('UPDATE Personal SET EstjVneshnieManip=1, Prover_vodit=1 where'+
     ...
     then
              ShowMessage('Неудачное оповещение об изменении общеиспользуемых данных!');
   end
  else
    begin
...
begin
  if not SendSQL('UPDATE Personal SET EstjVneshnieManip=0, Prover_vodit=0 where'+
    ' login='''+CurrUserLoginName+'''') then
              ShowMessage('Неудачный сброс признака наличия изменения общих данных для текущего пользователя!');
end;
...
begin
 ...
 if (IniFile.ReadString('удаленный_контроль',
  ...
  try
    ...
    ShowMessage('Неудачный запрос наличия изменения общих данных по текущему пользователю!');
  end;
  end
   else
     res:=False;
...
if IniFile.ReadString('BUSINESS_DATA','enable_for_allXML_DEMO','NO')='YES' then
 begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос на наличие номера в базе!');
  end;
 end;
Result:=res;
end;
        ...
        if UnionOADOTable.RecordCount>0 then
          begin
            ...
            if not SendSQL('UPDATE Objekt_vyborki_otchyotnosti SET Nom_putevogo_lista=Nom_putevogo_lista+1') then
              ShowMessage('Неудачная инкрементация номера путевого листа!');
            try
              UnionOADOTable.Active:=False;
              UnionOADOTable.Active:=True;
            except
              ShowMessage('Ошибка обновления таблицы настроек!');
            end;
          end
        else
          begin
            ShowMessage('Ошибка - пустая таблица настроек!');
          end;
      end
    else
      ShowMessage('Ошибка - недоступна таблица настроек!');
  end;
Result:=res;
end;
...
begin
  {try
    ...
    if BoldHandlesDM.blhSectorByCode.List.Count=1 then
      ...
      ShowMessage('Не найден сектор или секторов более одного с кодом - '+SectorByIDADOQuery.FindField('kod_sektora').AsString);
    end
      else
        begin
          if SID>0 then
           begin
            ShowMessage('В базе обнаружено более одного сектора или не одного, ID - '+IntToStr(SID));
           end; 
        end;
    ...
    if BoldHandlesDM.blhSectorByCode.List.Count=1 then
      ...
      ShowMessage('Не найден сектор или секторов более одного с кодом - '+SectorByIDADOQuery.FindField('kod_sektora').AsString);
        ...
        begin
          if EID>0 then
           begin
             ShowMessage('В базе обнаружено более одного сектора или не одного, ID - '+IntToStr(EID));
           end; 
        end;
  except
    ShowMessage('Неудачная процедура запроса сектора!');
  end;  }
end;
...
begin
  ...
  try
    ...
    if PersonaIDByLoginADOQuery.RecordCount=1 then
      ...
      ShowMessage('Ни одного или более двух пользователей с логином '+login);   

  except
    ShowMessage('Неудачный запрос системного ключа объекта Персона!');
  end;
  Result:=res;
end;
  ...
  if IniFile.ReadString('BUSINESS_DATA','enable_for_allXML_DEMO','NO')='YES' then
   ...
   begin
    try
      ...
      ShowMessage('Неудачный запрос количества незавершённых заявок по сотруднику!');
    end;
   end;
  Result:=res;
end;
    ...
    begin
      ...
      while not AllDriverADOTable.Eof do
        ...
        ReleaseStructForm.StatusBar2.Panels[0].Text:=
          TimeToStr(Time)+' На линии-'+IntToStr(ActiveCount)+
          ', занято-'+IntToStr(BusyCount)+
          ', на перерыве-'+IntToStr(OnLaunchCount)+
          ', свободно-'+IntToStr(ActiveCount-
            BusyCount-OnLaunchCount);
      end;
    end;
    except on E:Exception do
     begin
      ShowMessage('Неудачное прохождение запроса на обновление списка водителей. Ошибка: '+
        E.Message);
     end;
    end;
end;
...
res:=True;
 ...
 begin
  ...
  if blhCurrSyncDriver.List.Count=0 then
    begin
      ShowMessage('Одним из пользователей добавлен новый водитель, позывной - '
       ...
       if ((blhCurrSyncDriver.CurrentElement as TVoditelj).
         ...
         begin
           ...
           if BoldHandlesDM.blhSectorByCode.List.Count=1 then
             ...
             begin
               FirstForm.ShowAttention('Более одного сектора с одинаковым кодом , код - '
             ...
             begin
               FirstForm.ShowAttention('Одним из пользователей добавлен новый сектор, код - '
                + IntToStr(AllDriverADOTable.FindField(
                'Nomer_posl_sektora').AsInteger)+
                '. Либо у какого то водителя не установлен сектор. Установите сектор. Перезагрузите систему.');
               res:=False;
             end;
         end;
        end
    ...
    begin
      ShowMessage('Ошибка несколько водителей с одинаковым позывным - '
        + IntToStr(AllDriverADOTable.FindField(
        'pozyvnoi').AsInteger));
      Result:=False;
    end;
...
begin
  try
    ...
    ShowMessage('Неудачное выполнение запроса номера следующего заказа!');
  end;
  ...
  CurrTNumADOQuery.Parameters.Items[0].Value:=CustNum;
   try
    ...
    if CurrTNumADOQuery.RecordCount>0 then
     ...
     ShowMessage('Неудачный запрос номера телефона в базе!');
   end;
  end;
  Result:=res;
end;
...
begin
  ...
  'заголовок_главного_окна',
    ''))>0 then
    begin
      MainForm.FirstForm.Caption := IniFile.ReadString(
        'BUSINESS_DATA','заголовок_главного_окна',
        '');
    end;
...
begin
  ...
  try
    ...
    if CurrUserADOQuery.RecordCount<>1 then
      begin
        ShowMessage('Ни одного или более одного пользователя в таблице администрирования!');
      end;
  except
  end;
  Result:=login;
