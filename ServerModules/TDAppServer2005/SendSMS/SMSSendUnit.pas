...
begin
  ...
  try
          ...
          then
              begin
                ServerMainForm.WriteEvent('======'+IntToStr(CallRobotsADOQuery.FieldByName
                  ('CLIENT_CALL_STATE').AsInteger)+'=====ОТЗВОН РОБОТОМ');
                        ...
                        begin
                          if not ServerMainForm.SendLocalSQL('UPDATE Zakaz SET CLIENT_CALL_STATE=-1 '+
                            'WHERE BOLD_ID=' + IntToStr(CallRobotsADOQuery.
                              ...
                              'Установка признака ошибки отзвона НА ТОЧКЕ!') then
                            begin
   ...
   begin
    ServerMainForm.WriteEvent('Ошибка выполнения запроса списка' +
      ' заказов на отзвон, ошибка: '+E.Message+'.');
   end;
  end;
  finally
