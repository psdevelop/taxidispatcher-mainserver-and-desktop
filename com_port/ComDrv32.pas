...
uses
  Windows, Messages, SysUtils, Classes, Dialogs, Forms, ShellAPI;
...
begin
  ...
  tms.WriteTotalTimeoutMultiplier := 30;
  { Set WriteTotalTimeoutConstant: Specifies the constant, in milliseconds,
    used to calculate the total time-out period for write operations. }
  tms.WriteTotalTimeoutConstant := 6000;
  { Apply timeouts }
...
begin
  ...
  begin
    ...
    begin
      ...
      nSent:=0;
      if
        WriteFile( FComPortHandle, DataPtr^, nToSend, nSent, nil) then
         begin
           //ShowMessage('Команда: '+DataPtr+' ОК; Передано: '+IntToStr(nSent)+' байт.');
         end
        else
         ShowMessage('Не могу записать в порт ' + string(DataPtr));
    ...
    ShowMessage(' Buffer is full. If we are waiting too long then exit');
