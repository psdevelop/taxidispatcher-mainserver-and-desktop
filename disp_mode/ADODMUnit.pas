...
type
  ...
  TADOConnectDM = class(TDataModule)
    ...
    OrdOptsADODSstart_time: TWideStringField;
    OrdOptsADODSend_time: TWideStringField;
    LookOptionsDS: TDataSource;
    LookOptionsADOT: TADOTable;
    LookOptionsADOTID: TAutoIncField;
    LookOptionsADOTOPTION_NAME: TStringField;
    LookOptionsADOTOSUMM_COEFF: TBCDField;
    LookOptionsADOTOS_COMPOSED: TBCDField;
    LookOptionsADOTPR_POLICY_ID: TIntegerField;
    LookOptionsADOTSHORT_NAME: TStringField;
    LookOptionsADOTIF_DEF: TSmallintField;
    LookOptionsADOTstart_time: TWideStringField;
    LookOptionsADOTend_time: TWideStringField;
...
uses AnapaMapUnit, DateUtils, MainForm, ReleaseStructUnit, BusinessClasses, HandlesDM,
 ...
 begin
   ...
   //if (IniFile.ReadString('BUSINESS_DATA','озвучивать_онлайн_заказы','NO')='YES') then
...
begin
  ...
  try
    ...
    begin
      ShowMessage('Ошибка получения параметров из БД! '+
        E.Message);
      FirstForm.Close;
    end;
    end;
  ...
  try
      ...
      AdrLiveSearchADODS.CommandText:=
        ...
        'Adres_vyzova_vvodim LIKE '''+like_s+'%'''+
        ' UNION '+
        ' SELECT ks.[Столбец 0] as Adres_vyzova_vvodim '+
        ' FROM KLADR_KLADR kk JOIN KLADR_STREET ks ' +
        'ON kk.[Столбец 6]=ks.[Столбец 6] ' +
        ' WHERE kk.[Столбец 2]='''+kladr_code+''' '+
        ' AND ks.[Столбец 0] LIKE '''+like_s+'%'') tt '+
        ' ORDER BY tt.Adres_vyzova_vvodim ASC';
      AdrLiveSearchADODS.Active:=True;
  except on E:Exception do
    ShowMessage(
      'Неудачное обновление списка live-adress'+
...
begin
  try
    ...
    ShowMessage(
      'Неудачное обновление списка групп вод с тп'+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное обновление списка вод с тп '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное обновление списка тарифных планов '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное обновление таблицы тарифных планов '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное обновление таблицы тарифов '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное обновление таблицы опций заказов '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное выполнение RealizeEventADOSP '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное выполнение ReservePrOrdADOSP '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное выполнение RealizeAbPresADOSP '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное выполнение ResetRemDrFlADOSP '+
      E.Message);
  end;
end;
...
begin
  try
    ...
    ShowMessage(
      'Неудачное выполнение RestoreRemDrFlADOSP '+
      E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    ShowMessage(
      'Неудачный запрос GetDrLockOnCalcDebtDynADOSP статуса '+
      'блокировки по оплате водителя! '+E.Message);
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    begin
      if ActiveEventsADODS.FieldByName
        ...
        FirstForm.ShowAttention(
          'Один из водителей подал сигнал ТРЕВОГИ!!!');
      if (ActiveEventsADODS.FieldByName
    ...
    ShowMessage('Ошибка перезагрузки активных событий! '+E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Ошибка установки статуса запроса GPS-координат! '+E.Message);
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Ошибка установки статуса обычной заявки! '+E.Message);
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    if (DrPhoneByNumADOQuery.RecordCount=1) then
      ...
      begin
        ShowMessage('Не найден водитель в базе или их более одного с одним позывным!');
      end;
  except on E: Exception do
    begin
      ShowMessage('Неудачное выполнение запроса номера телефона водителя! ' + E.Message);
    end;
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    if (OrderSMSStatesADOQuery.RecordCount>0) then
      ...
      begin
        ShowMessage('Не найдена заявка в базе, возможно она удалена другим пользователем!');
        res:=-2;
      end;
  except
    ShowMessage('Неудачное выполнение запроса состояния отправки SMS!');
  end;
  Result:=res;
end;
...
begin
  if not SendSQL('UPDATE Zakaz SET DRIVER_SMS_SEND_STATE=1, SMS_SEND_DRNUM = '''
    +PhoneNumber+''' WHERE BOLD_ID='+IntToStr(OrderID)) then
                ShowMessage('Неудачная установка запроса отправки SMS водителю!');
end;
...
begin
  if not SendSQL('UPDATE Zakaz SET CLIENT_SMS_SEND_STATE=1 '+
        ' WHERE BOLD_ID='+IntToStr(OrderID)) then
                ShowMessage('Неудачная установка запроса отправки SMS клиенту!');
...
begin
  if not SendSQL('UPDATE Zakaz SET CLIENT_SMS_SEND_STATE=3 '+
        ' WHERE BOLD_ID='+IntToStr(OrderID)) then
                ShowMessage('Неудачная установка запроса отправки отчета по SMS клиенту!');
end;
...
begin
  ...
  if (IniFile.ReadString('удаленный_контроль','вести_удаленный_контроль_сокет_клиентов','NO')='YES') then
  begin
...
begin
  ...
  if (IniFile.ReadString('BUSINESS_DATA',
    'установливать_признак_призовой_без_диалога','NO')='YES') then
    res:=True
  else
...
try
 ...
 try
   ...
   begin
     if not SendSQL('UPDATE Zakaz SET konechnyi_sektor_raboty='+IntToStr(find_sect_bold_id)+
       ', adr_manual_set=1 WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
           ShowMessage('Неудачная установка сектора окончания заявки!');
   end
   else
    begin
      if (IniFile.ReadString('удаленный_контроль',
	      'вести_удаленный_контроль_сокет_клиентов','NO')='YES') and
       ...
       ', REMOTE_SET=2, adr_manual_set=1, Priority_counter=0 WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
           ShowMessage('Неудачная установка сектора рассыла заявки!');
		  end;
    end;
 except
   ShowMessage('Неудачный запрос сектора по наименованию!');
 end;
  ...
  begin
    ...
    ShowMessage('Операция отменена, повторите попытку заново!');
  end;
...
begin
  try
    ...
    ShowMessage('Запись из черного списка удачно удалена: номер - '+
      phone+'.');
  except
    ShowMessage('Ошибка удаления записи из черного списка!');
  end;
end;
    ...
    if RelateAdrCountADOQuery.RecordCount>0 then
      begin
        try
          ...
          ShowMessage('Новый адрес успешно установлен!');
        except
          ShowMessage('Неудачное прохождение запроса на установку соответствия нового адреса телефонному номеру!');
        end;
      end
    else
      ShowMessage('В таблице соответствий адресов и телефонных номеров нет записей с таким телефонным номером!');
  except
    ShowMessage('Неудачное прохождение запроса на наличие соответствия адреса телефонному номеру!');
  end;
end;
...
begin
  ...
  try
   ...
   begin
    ShowMessage('Неудачное прохождение запроса текущего времени к серверу! Ошибка '+E.Message+'.');
   end;
  end;
end;
  ...
  if ActiveCustADOQuery.RecordCount>0 then
    begin
      if ActiveCustADOQuery.FieldByName('Predvariteljnyi').AsInteger<>0 then
        begin
          if MessageDlg('Отменить предварительный заказ?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
            begin
              ...
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачное снятие признака предварительной заявки!');
        ...
        begin
          if not SendSQL('UPDATE Zakaz SET Predvariteljnyi=1'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка признака предварительной заявки!');
          ...
          if not SendSQL('UPDATE Zakaz SET Data_predvariteljnaya=:date_prm'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка даты предварительной заявки!');
          RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
        end;
    end
  else
    ShowMessage('Нет ни одной заявки!');
end;
      ...
      begin
        ...
        if not SendSQL('UPDATE Zakaz SET Zadeistv_predvarit=1'+
              ' WHERE BOLD_ID='+IntToStr(PrevCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка признака активности предварительной заявки!');
        ...
        while not PrevCustADOQuery.Eof do
         ...
         ShowMessage('Ищите предварительные заявки, помеченные жёлтым цветом!');
         FirstForm.ShowAttention('',True);
      end;
  except on E:Exception do
  begin
    ShowMessage('Неудачное прохождение запроса на наличие активных предварительных заявок!'+
      E.Message);
  end;
  end;
 end;       
    ...
    begin
      ...
      //  begin
          ...
          //  begin
              ...
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка статуса онлайн-заказа!');
              RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
          //  end
      //  end
    end
...
begin
  if ActiveCustADOQuery.RecordCount>0 then
    begin
      if ActiveCustADOQuery.FieldByName('Slugebnyi').AsInteger<>0 then
        begin
          if MessageDlg('Снять служебный заказ?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
            begin
              if not SendSQL('UPDATE Zakaz SET Slugebnyi=0'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачное снятие признака служебной заявки!');
              RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
            end
        end
      else
        begin
          if not SendSQL('UPDATE Zakaz SET Slugebnyi=1'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка признака служебной заявки!');
          RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
        end;
    end
  else
  ...
  if ActiveCustADOQuery.RecordCount>0 then
    begin
      if ActiveCustADOQuery.FieldByName('call_it').AsInteger<>0 then
        begin
          if MessageDlg('Снять вызов для заказа?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
            begin
              if not SendSQL('UPDATE Zakaz SET call_it=0'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачное снятие признака вызова заявки!');
              RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
            end
        end
      else
        begin
          if not SendSQL('UPDATE Zakaz SET call_it=1'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка признака вызова заявки!');
          RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
        end;
    end
  else
  ...
  if ActiveCustADOQuery.RecordCount>0 then
    begin
      if ActiveCustADOQuery.FieldByName('Nomer_skidki').AsInteger=-3000 then
        begin
          if MessageDlg('Отменить заказ-услугу?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
            begin
              if not SendSQL('UPDATE Zakaz SET Nomer_skidki=0'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачное снятие признака заявки на услугу!');
              RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
            end
        end
      else
        begin
          if not SendSQL('UPDATE Zakaz SET Nomer_skidki=-3000, adres_vyzova_vvodim='''+
              IniFile.ReadString('BUSINESS_DATA','service_append_word','УСЛУГА')+' ''+adres_vyzova_vvodim'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачная установка признака служебной заявки!');
          RefreshMainGrid(ActiveCustADOQuery.FindField('Nomer_zakaza').AsInteger);
        end;
...
begin
  ...
  ADOConnectDM.SectByCodeADOQuery.Parameters[0].Value:=code;
    ...
    if (ADOConnectDM.SectByCodeADOQuery.RecordCount=1) then
       ...
       ShowMessage('Неудачное ивлечение ключа по коду сектора!');
    end;
   Result:=res;
end;
   ...
   try
    ...
    if DrInfoADOQuery.RecordCount>1 then
      begin
        ...
        ShowMessage('Более чем один или не одного водителя с позывным '+IntToStr(DrNum));
      end
       else
         if DrInfoADOQuery.RecordCount=1 then
          res:=ADOConnectDM.DrInfoADOQuery.FindField('BOLD_ID').AsInteger;
  except
    ShowMessage('Неудачный запрос водителя по позывному - '+InttoStr(DrNum));
  end;
   Result:=res;
end;
...
if (BoldModelDM.HasAllBonusADOQuery.FieldByName(
                      ...
                      (IniFile.ReadString('BUSINESS_DATA','disable_repeat_enter','NO')<>'YES') then
                         begin
                              ...
                              if adr<>'' then begin
                                    ...
                                    //else
                                    //  ShowMessage(sql_text+'удачная установка параметров заявки!');
  ...
  ShowMessage('Ошибка установки адреса! '+E.Message);
end;
...
begin
 ...
 begin
   ...
   try
        ...
        //Обновление после определенного номера
                  ...
                  begin
                    ...
                    if not NotJumpWithLastChange then
                      begin
                        ...
                        if (IniFile.ReadString('BUSINESS_DATA','срыть_поле_региона_или_аб_номера','NO')='NO') then
        ...
        if (IniFile.ReadString('BUSINESS_DATA','озвучивать_онлайн_заказы','NO')='YES') then begin
      ...
      begin
        ShowMessage('Неудачное обновление таблицы заявок диспетчера! Ошибка'+E.Message+'.'+ActiveCustADOQuery.SQL.Text);
        ...
        try
         ...
         begin
          ShowMessage('Неудачное повторное обновление таблицы заявок диспетчера! Ошибка'+E.Message+'.');
         end
        end;
   ...
   if (IniFile.ReadString(
      'BUSINESS_DATA','вести_контр_событий',
      'NO')='YES') then
        begin
          //showmessage('11');
...
//Конец условия на повторный запуск процедуры
end;
...
begin
  try
    ...
    ShowMessage('Неудачное обновление удаленных '+
    ...
    begin
        ...
        case status of
            ...
            ORDER_IS_OCCUPED:
              begin
                ...
                if (IniFile.ReadString('удаленный_контроль',
                'спрашивать_при_удаленной_пост_на_заказ',
                'NO')='YES') then
                  allowOperation:=PresentMessageDlg('Установить водителя '+
                    RemSetOrdersADODS.FieldByName('REMOTE_DRNUM').
                    AsString+' на заявку '+RemSetOrdersADODS.
                    FieldByName('Nomer_zakaza').AsString+'? Нет - '+
                    'выбор из претендентов.');
      ...
      ORDER_ONHAND_ATTEPMT:
        begin
                  allowOperation:=True;
                if (IniFile.ReadString('удаленный_контроль',
                'спрашивать_при_постановке_с_руки_водителем',
                'NO')='YES') then
                  allowOperation:=PresentMessageDlg('Установить водителя '+
                    RemSetOrdersADODS.FieldByName('REMOTE_DRNUM').
                    AsString+' c руки на заявку '+RemSetOrdersADODS.
...
begin
  ...
  try
    ...
    if (IniFile.ReadString('BUSINESS_DATA',
          'не_пускать_к_БД_с_одним_именем','NO')='YES')  then
    begin
    try
        ...
        begin
          if ActiveDBConnsADODS.FieldByName('Login').AsString=
            ...
            begin
              ...
              if usr_cnt>1 then
              begin
              ShowMessage('Пользователь с таким именем ('+
              ActiveDBConnsADODS.FieldByName('Login').AsString+') подключен к базе данных!');
              res:=true;
              Break;
              end;
            end;
      ...
      ShowMessage('Неудачный запрос списка пользователей, подключенных к базе данных!');
    end;
    end;
    ...
    if CurrUserADOQuery.RecordCount=1 then
      begin
       ...
       if SetDispID then
        ...
        if (IniFile.ReadString('BUSINESS_DATA',
          'не_показывать_неактивированные_предварительные_заявки','NO')='YES')  then
          FirstForm.PrevOrdersMenu.Enabled := True;

        if not (IniFile.ReadString('BUSINESS_DATA','filter_by_manual_ready','NO')='YES') and
           (IniFile.ReadString('BUSINESS_DATA','не_показывать_неактивированные_предварительные_заявки','NO')='YES')  then
          begin
          ...
          begin
           if (IniFile.ReadString('BUSINESS_DATA','не_показывать_неактивированные_предварительные_заявки','NO')='YES') then
      ...
      begin
        ShowMessage('Ни одного или более одного пользователя с логином '
        +USRLoginADOQuery.FindField('curr_usr_name').AsString);
      end;
  except
    ShowMessage('Неудачное проведение запроса имени текущего пользователя!');
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачное выполнение инструкции на поиск признака отметки в удаленной базе!');
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    begin
      res:=-1;
      ShowMessage('Неудачный запрос количества активных интернет-заявок! '+E.Message);
    end;
  end;
...
if DrNum>0 then
    ...
    begin
      PrevOnLaunch:=OnLaunch(DrNum);
			...
			begin
				ShowMessage('Данная машина находится на перерыве!');
                Result:=False;
			  Exit;
			end;
    end;
	...
	if (IniFile.ReadString('BUSINESS_DATA',
		'снимать_должн_при_уст_поз_по_нединам_подсч',
		'NO')='YES') then
    begin
			...
			begin
				BoldModelDM.SendSQL('UPDATE Zakaz SET Pozyvnoi_ustan=0 WHERE BOLD_ID='+
					...
					'BOLD_ID').AsString);
				ShowMessage('Данный водитель в должниках!');
                Result:=False;
			  Exit;
			end;
    end;
    ...
    if (IniFile.ReadString('BUSINESS_DATA',
		'снимать_должн_при_уст_поз_по_динамическому_подсч',
		'NO')='YES') then
    begin
			...
			begin
				BoldModelDM.SendSQL('UPDATE Zakaz SET Pozyvnoi_ustan=0 WHERE BOLD_ID='+
					ActiveCustADOQuery.FindField(
					'BOLD_ID').AsString);
				ShowMessage('Данный водитель в должниках по дин. балансу!');
                Result:=False;
			  Exit;
			end;
    end;
    ...
    if (ActiveCustADOQuery.
				...
				begin
          if (IniFile.ReadString('BUSINESS_DATA',
		        'сообщать_о_пустом_телефоне',
		        'YES')='YES') then
					ShowMessage('Не установлен телефон - отправка в базу обмена невозможна!');
          //Exit;
				end;
      ...
      Exit;
		...
		//Главный блок постановки если найден водитель
    ...
    begin
      ...
      and PrevOnLaunch then
			begin
				ShowMessage('Водитель поставил себя на перерыв, будет отослана инструкция о снятии!');
			  //Exit;
			end;

      HasPrevDrv:=False;
      PrevDrvNum:=0;
      //Проверка на наличие адреса и заранее установленного позывного
      ...
      begin
        if (IniFile.ReadString('удаленный_контроль',
			    'вести_удаленный_контроль_сокет_клиентов','NO')='YES') and
          (IniFile.ReadString('удаленный_контроль',
			    'предлагать_обычную_учетную_для_удаленного','NO')='YES') and
          (DrInfoADOQuery.FindField('ITS_REMOTE_CLIENT').AsInteger=1)
			  then
          if not self.PresentMessageDlg('Запустить отправку'+
            ' водителю? Нет - заявка оформится как обычная учетная.')
            then
              DenyRemoteOrder:=True;
      end;

			//Блок формирования доп инструкции уд упр-я
			REMOTE_SET_SQL:='';
			if (IniFile.ReadString('удаленный_контроль',
			'вести_удаленный_контроль_сокет_клиентов','NO')='YES') and
      ...
      begin
          ...
          //then
            ...
            if (IniFile.ReadString('удаленный_контроль',
						'гибридный_режим_назначения_удал_водителя',
						'NO')='YES')
						then
            begin
              REMOTE_SET_SQL:=
              ...
              if (IniFile.ReadString('удаленный_контроль',
						    'признак_подтверждения_у_индивид_назначеных',
						    'NO')='YES')
						  then
              ...
              ', Pozyvnoi_ustan=0, REMOTE_SET=1, REMOTE_INCOURSE=0'+
						...
						if (IniFile.ReadString('удаленный_контроль',
						'заказ_как_индивидуальный_при_введ_позывного',
						'NO')='YES')
						then REMOTE_SET_SQL:=
            ...
            if not SendSQL('UPDATE Zakaz SET otpravlyaetsya='+
              ...
              //IntToStr(DrInfoADOQuery.FindField
				      ...
				      ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
					      ShowMessage('Неудачная установка связи'+
					      ' заявки с диспетчером отправки, либо'+
					      ' переменной позывного заявки, либо водителя на заявку!');

					//end
					//else
					//	ShowMessage('Данная заявка уже'+
					//	' отправлена в базу обмена! Дождитесь'+
					//	' завершения рассылок или отказа и отправьте повторно.');

      end
      else
      //Конец блока формир инстр уд-го упр-я
      begin
      //Если пытаются назначить водителя без приложения
      //или нулеввого
      ...
      REMOTE_SET_SQL:=', REMOTE_SET=0, REMOTE_INCOURSE=0, REMOTE_SYNC=0'+
        ...
        //Блок посылки основной инструкции для вод. без приложения
      ...
      //else
            ...
            begin
                ...
                //Блок отправки СМС клиенту
				//Не отправляем если работа в режиме сокетов
				if (IniFile.ReadString('удаленный_контроль',
                    'автоматически_отправлять_SMS_клиенту_при_введении_позывного',
						...
						if (SMSSendState<>1) and
                            ...
                            begin
                              //DrPhone:=GetDrPhoneByDrNum(ActiveCustADOQuery.FindField('Pozyvnoi_ustan').AsInteger);
								...
								ShowMessage('SMS по данной заявке'+
								' уже отправляется системой, дождитесь результата!');

                    end;


                end;
				//Конец отправки СМС клиенту

                //Блок отправки СМС водителю
				//Не отправляем если работа в режиме сокетов
				if (IniFile.ReadString('удаленный_контроль',
                    'автоматически_отправлять_SMS_водителю_при_введении_позывного',
					'NO')='YES') and (ActiveCustADOQuery.FindField
        ('NO_TRANSMITTING').AsInteger=0) then
                begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                ShowMessage('Неподходящий номер'+
								' телефона водителя ''' + DrPhone + ''', указанный'+
								' в справочнике водителей!');
                        end
                            else
                                ShowMessage('SMS по данной заявке'+
			...
			//Конец блока посыла осн инструкции
      end;

		end;
		//Конец главного блока пост если найден водитель

    end
  //Блок если выставили нулевой позывной
  else
	begin
		HasPrevDrv:=False;
    PrevDrvNum:=0;
    ...
    REMOTE_SET_SQL:=', REMOTE_SET=0, REMOTE_INCOURSE=0, REMOTE_SYNC=0'+
		...
		if not SendSQL('UPDATE Zakaz SET vypolnyaetsya_voditelem=-1'+
            ...
            AsInteger)) then
                ShowMessage('Неудачная установка пустой связи с водителем!');
			...
			begin
				...
				if not SendSQL('UPDATE Voditelj SET Zanyat_drugim_disp=0'+
                    ' WHERE BOLD_ID='+IntToStr(PrevDrvNum)) then
                        ShowMessage('Неудачная установка признака'+
						' незанятости прежнего водителя!');
    ...
    //        ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.
		//	FindField('BOLD_ID').AsInteger)) then
    //            ShowMessage('Неудачная установка нулевого позывного!');
	end;
	//Конец блока установки нулевого позывного

except on E:Exception do
  ShowMessage('Ошибка назначения водителя! '+E.Message);
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный поиск водителя по ключу!');
  end;
  Result:=res;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос числа незавершённых заявок по выбранному водителю!');
  end;
  Result:=res;
end;
...
if (Sale>0) or EnableSpecZeroSet then
	begin
      ...
      begin
        if (IniFile.ReadString('удаленный_контроль',
						'гибридный_режим_отчета_удал_заявки',
						'NO')='YES')
        then
        begin
          if (IniFile.ReadString('удаленный_контроль',
						'спрашивать_при_ручном_отч_удал_заявки',
						'YES')='NO')
            then
            allowRemClose:=True;
          if not allowRemClose then
          if self.PresentMessageDlg('Данная заявка выполняется удаленно, '+
            'завершить вручную?') then
            allowRemClose:=True;
          if allowRemClose then
            if not SendSQL('UPDATE Zakaz SET Zavershyon=1'+
                ...
                'признака завершения заявки!');
        end;
      end
      else if (IniFile.ReadString('удаленный_контроль',
			'вести_удаленный_контроль_сокет_клиентов','NO')='YES') and
        (remote_set>0) and (remote_set<8) then
      begin
        ShowMessage('Заявка на этапе раздачи!');
      end
      else
      begin
			    if not SendSQL('UPDATE Zakaz SET Zavershyon=1'+
      ...
      begin
        ShowMessage('Нет связи с водителем!');
        Result:=False;
        Exit;
      end;
  ...
  ShowMessage('Ошибка установки отчета! '+E.Message);
end;
...
begin
  ...
  if IniFile.ReadString('BUSINESS_DATA',
   ...
   begin
    ...
    try
      ShowMessage('Дата подсчета для водителей: '+DateToStr(StrToDate(
        IniFile.ReadString('BUSINESS_DATA',
        'дата_начала_контроля_оплат_водителей','NO'))));
    except
      ShowMessage('Неверно выставлена дата подсчета для водителей!');
    end;
  ...
  if (IniFile.ReadString('BUSINESS_DATA',
    'предлагать_учет_по_номеру_абонента','NO')='NO') then
    begin
      N9.Visible:=False;
    end;
  ...
  if (IniFile.ReadString('удаленный_контроль',
    'запретить_отсылку_SMS_водителям','YES')='YES') then
      begin
...
begin
 try
  StrToInt(i_num);
  if (IniFile.ReadString('BUSINESS_DATA',
         'выводить_посл_заявки_абонента','NO')='YES') then
           begin
...
if ReleaseStructForm.DispDBGrid.SelectedField<>nil then
 ...
 begin
  ...
  if CurrQueryFieldName='Uslovn_stoim' then
    ...
    begin
     if (IniFile.ReadString('BUSINESS_DATA',
        'разрешить_забивать_учетный_номер_абонента_напрямую_в_таблицу','NO')='YES') then
      begin
  ...
  begin
    ShowMessage('Идёт обновление данных!');
    ActiveCustADOQuery.CancelUpdates;
  end;
except
  FirstForm.StatusBar.Panels[4].Text:=
     'Ошибка обработки onAfterPost!';
...
begin
//Отсчет времени с последнего вызова сотового телефона
...
begin
  if ActiveCustADOQuery.RecordCount>0 then
    begin
       if MessageDlg('Переместить заказ в архив?', mtConfirmation,[mbYes,mbNo],0) = 6 then
      begin

      if MessageDlg('Записать как нежелательный?', mtConfirmation,[mbYes,mbNo],0) = 6 then
       begin
      ...
      if not SendSQL('UPDATE Zakaz SET Zavershyon=1, Adres_vyzova_vvodim=''ОТМЕНА ''+Adres_vyzova_vvodim, Konec_zakaza_data=(CAST(GETDATE() as DateTime)) '+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачное перемещение заявки в архив!');
...
begin
  if ActiveCustADOQuery.RecordCount>0 then
    begin
      ...
      N15.Enabled:=False;
      N16.Enabled:=False;
      if not (IniFile.ReadString('удаленный_контроль','вести_удаленный_контроль','NO')='YES')       then
        begin
          Internet1.Enabled:=False;
        end;
      SMS1.Enabled:=True;
      SMS2.Enabled:=True;
      if (IniFile.ReadString('удаленный_контроль','запретить_отсылку_SMS_водителям','YES')='YES') and not
      (IniFile.ReadString('удаленный_контроль','разрешить_вручную_отправлять_SMS_водителям','NO')='YES')  then
        begin
          SMS2.Enabled:=False;
        end;
      if (IniFile.ReadString('удаленный_контроль','разрешить_вручную_отправлять_SMS_клиентам_о_напр_машине','NO')='NO') then
        begin
          SMS1.Enabled:=False;
        end;
      if (IniFile.ReadString('удаленный_контроль','разрешить_вручную_отправлять_SMS_водителям','NO')='NO')       then
        begin
          SMS2.Enabled:=False;
        end;
      if ActiveCustADOQuery.FieldByName('Predvariteljnyi').AsInteger<>0 then
        begin
        DispCustToArhivePopupMenu.Items[1].Caption:='Снять предварительный заказ...';
        ...
        DispCustToArhivePopupMenu.Items[1].Caption:='Пометить предварительный заказ...';
      if ActiveCustADOQuery.FieldByName('Slugebnyi').AsInteger<>0 then
        DispCustToArhivePopupMenu.Items[2].Caption:='Снять служебный заказ...'
      else
        DispCustToArhivePopupMenu.Items[2].Caption:='Пометить служебный заказ...';
...
begin
  ...
  if ActiveCustADOQuery.RecordCount>0 then
   ...
   FirstForm.StatusBar.Panels[4].Text:=
     'Ошибка обработки onBeforeEdit!';
 end;
end;

procedure TADOConnectDM.N5Click(Sender: TObject);
begin
  if MessageDlg('Отменить призовую заявку?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
            begin
              if not SendSQL('UPDATE Zakaz SET Nomer_skidki=0'+
              ' WHERE BOLD_ID='+IntToStr(ActiveCustADOQuery.FindField('BOLD_ID').AsInteger)) then
                ShowMessage('Неудачное снятие признака заявки на услугу!');
  ...
  if (BoldModelDM.HasAllBonusADOQuery.FieldByName('Ispoljz_priz_1').AsInteger<>0) or
     ...
     begin
       ...
       ReleaseStructForm.sLabel1.Caption:=
         'Призовое накопление по тел. номеру: требуется-'+
         IntToStr(CallCount)+', всего у абонента-'+
         IntToStr(BonusCount)+', на данный момент-'+
         IntToStr((BonusCount mod CallCount))+', осталось-'+
 ...
 try
  ...
  if (BoldModelDM.HasAllBonusADOQuery.FieldByName('Ispoljz_priz_1').AsInteger<>0) or
     ...
     begin
       ...
       ReleaseStructForm.sLabel1.Caption:=
         'Призовое накопление по аб. номеру: требуется-'+
         IntToStr(IndCallCount)+', всего у абонента-'+
         IntToStr(IndBonusCount)+', на данный момент-'+
         IntToStr((IndBonusCount mod IndCallCount))+', осталось-'+
...
begin
  ShowMessage('По вопросам приобретения данной функции обр. к разработчику!');
end;
...
begin
 if ReleaseStructForm.DispDBGrid.SelectedField.FieldName='Pozyvnoi_ustan' then
      begin
        ...
        if (IniFile.ReadString('BUSINESS_DATA',
        'контроль_смены_позывного','NO')='YES') then
        begin
        if MessageDlg('Записать позывной поверх установленного?',
          mtConfirmation,[mbYes,mbNo],0) = mrYes then
          begin
          end
          else  begin
        ...
        'запрет_смены_позывного','NO')='YES') then
        begin
          Abort;
          ShowMessage('Запись позывного поверх установленного запрещен!');
        end;
 ...
 if ReleaseStructForm.DispDBGrid.SelectedField.FieldName='Uslovn_stoim' then
   ...
   (ADOConnectDM.ActiveCustADOQuery.FieldByName('Zadeistv_predvarit').AsInteger=0) then
        begin
          ShowMessage('Вы пытаетесь проставить отчет в незадействованной предварительной заявке!');
          DataSet.FindField('Uslovn_stoim').AsFloat:=0;
        end
    else
    begin
     if (IniFile.ReadString('BUSINESS_DATA',
        'подтверждать_заверяемую_сумму_отчета','NO')='YES') then
        begin
          if (DataSet.FindField('Uslovn_stoim').AsFloat>=
               IniFile.ReadFloat('BUSINESS_DATA',
                 'заверяемая_сумма_отчета',150)) then
             begin
                if MessageDlg('Данная сумма выше или равна заверяемому барьеру, установить отчет?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
                  begin
                  end
                else
                  begin
  ...
  if (BoldHandlesDM.blhDriversUnion.CurrentElement as 
    ...
    begin
      ...
      if not ModemSettingsForm.ModemsOnActivePhoneLine then
        begin
          ShowMessage('Модем не подключен к линии! Проверьте наличие гудка, подключенность розеток к модему и перезагрузите программу!');
          FirstForm.Close;
        end;
      ...
      if not ModemSettingsForm.GSMModemsOnActivePhoneLine then
        begin
          if (IniFile.ReadString('BUSINESS_DATA','выключать_программу_при_отсутствии_модема','NO')='YES') then
            begin
              ShowMessage('Модем сотового телефона не подключен к линии! Проверьте подключенность соответствующего гнезда телефона к компьюьеру или активность беспроводного подключения к копьютеру и перезагрузите программу!');
              FirstForm.Close;
            end
          else
            begin
              FirstForm.StatusBar.Panels[4].Text :=
                'Модем GSM недоступен!!!';
              if (IniFile.ReadString('BUSINESS_DATA','производить_попытки_восстановления_связи_с_модемами','NO')='YES') then
                begin
                      ...
                      if not ModemSettingsForm.GSM2ApdComPort.Open and
                        ...
                        begin
                          if (IniFile.ReadString('BUSINESS_DATA',
                             'перезагружать_все_USB_устройства_при_неудачном_восстановлении_связи','NO')='YES') and
                             (IniFile.ReadString('BUSINESS_DATA',
                             'перезагружать_все_USB_устройства_после_модемов','NO')='NO') and
                             not AllUSBReset 
                               then
                                 begin
                                   if ChangeDeviceState(DICS_DISABLE, True) then
  ...
  if (ActiveCustADOQuery.FindField('Adres_vyzova_vvodim').AsString='') or
     ...
     not (ActiveCustADOQuery.FindField('Pozyvnoi_ustan').AsInteger>0) or
      (ActiveCustADOQuery.FindField('Telefon_klienta').AsString='') then
        begin
          ShowMessage('Не установлен адрес, телефон или нет связки с объектом водителя (попробуйте заново ввести позывной)!');
          ActiveCustADOQuery.CancelUpdates;
        end
      else
        begin
          ...
          (ActiveCustADOQuery.FindField('DRIVER_SMS_SEND_STATE').AsInteger<>1) then
            begin
              ...
              if (StrToInt64Def(DrPhone,-1)<>-1) and
                ...
                ShowMessage('Неподходящий номер телефона водителя ''' + DrPhone + ''', указанный в справочнике водителей!');
            end
          else
            ShowMessage('SMS по данной заявке отправляется системой, дождитесь результата!');
  ...
  if (ActiveCustADOQuery.FindField('Adres_vyzova_vvodim').AsString='') or
     ...
     not (ActiveCustADOQuery.FindField('Pozyvnoi_ustan').AsInteger>0) or
      ...
      (ActiveCustADOQuery.FindField('BOLD_ID').AsInteger
                ...
                ShowMessage('Неподходящий номер телефона клиента!');
            end
          else
            ShowMessage('SMS по данной заявке отправляется системой, дождитесь результата!');
...
begin
  try
    ...
    begin
      ...
      ShowMessage('Выгрузка произведена!');
    end;
  except on E:Exception do
    ShowMessage('Неудачный запрос выборки аб. номеров на экспорт! Сообщение: '+E.Message);
  end;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ShowMessage('Не обнаружено данных для загрузки!');
    end
    else
      ShowMessage('Неактивный набор для импорта, неудачная загрузка!');
    end;
...
begin
  try
    ...
    ShowMessage('Неудачный запрос выборки телефонного справочника на экспорт! Сообщение: '+E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос телефонного справочника на экспорт! Сообщение: '+E.Message);
  end;
  Result:=res;
end;
...
begin
  ...
  if (IniFile.ReadString('BUSINESS_DATA',
    'вести_контроль_оплаты_водителем','NO')='YES')
   ...
   begin
    ...
    try
      ...
      if emploeeDate>Today then
        ShowMessage('Неверно выставлена дата приема водителя!');
      ...
      DrWorkSummADODS.Parameters.
       ...
       ShowMessage('Сегодня: '
       +DateToStr(Today)+'. Считаем с '+
       DateToStr(emploeeDate)+' , отпускных: '+
       IntToStr(freeDaysCount)+'. Сдал с '+
       DateToStr(start_date)+': '+
        ...
        .FieldByName('DrSumm').asFloat)
          ...
          begin
            ShowMessage('Неудачный запрос к таблице водителей!');
          end;
...
begin
if IniFile.ReadString('удаленный_контроль',
  'вести_удаленный_контроль_сокет_клиентов','NO')='YES' then
  begin
    if IniFile.ReadString('удаленный_контроль',
    'автозапрос_состояний_сокет_клиентов','NO')='YES' then
      begin
        RefreshRemoteData;
      end;  
  end;
  ...
  begin
    ...
    begin
      try
        ...
        ShowMessage('Ошибка обновления таблицы'+
          ' претендентов! '+E.Message);
      end;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      try
		...
		if (count<1) then
			ShowMessage('Не найдено заявки с поодходящим статусом!');
      except on E: Exception do
      end;
    end
    else
      ShowMessage('Пустая таблица претендентов!');
  end
  else
    ShowMessage('Неактивна таблица претендентов!');
end;
      ...
      try
        ...
        ManualSetOrdRemStatADOSP.ExecProc;
		    count:=ManualSetOrdRemStatADOSP.Parameters.
                ParamByName('@count').Value;
		  if (count<1) then
			  ShowMessage('ОПЕРАЦИЯ НЕ ВЫПОЛНЕНА!!! '+
...
begin
  ...
  if ActiveCustADOQueryfixed_summ.AsFloat>0 then
   Text:='пч='+FloatToStr(ActiveCustADOQueryfixed_summ.AsFloat)+','+Text;
  if ActiveCustADOQuerytm_summ.AsFloat>0 then
   Text:='км='+FloatToStr(ActiveCustADOQuerytm_summ.AsFloat)+','+Text;
end;
    ...
    if SettingsADODS.RecordCount>0 then
      if SettingsADODS.FieldByName('db_version').
        AsInteger<MinDBVersion then
          begin
            ShowMessage('Неподходящая версия БД!');
            FirstForm.Close;
          end;
    except on E:Exception do
    begin
...
begin
  ...
  if ActiveCustADOQueryfixed_time.AsInteger>0 then
    Text:='(почас. '+IntToStr(ActiveCustADOQueryfixed_time.AsInteger)+
      ' мин.)'+Text;
  if ActiveCustADOQuerytm_distance.AsInteger>0 then
    Text:='(расст. '+FloatToStr(ActiveCustADOQuerytm_distance.asFloat/1000)+
      ' км.)'+Text;
...
begin
  try
    ...
    ShowMessage('Ошибка добавления к дин. балансу водителя! '+E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    ShowMessage('Неудачный запрос признака дин. подсчета водителя! '+E.Message);
  end;
  Result:=res;
end;
...
begin
  if (IniFile.ReadString('BUSINESS_DATA',
    'контроль_дин_баланса_руч_постановка','NO')='YES')
     then
  if (BoldModelDM.HasAllBonusADOQuery.FieldByName(
  ...
  begin
    ...
    'MIN_DEBET').AsFloat>ADODMUnit.CurrDrBalance) then
      ShowMessage('Водитель с позывным '+IntToStr(DrNum)+
        ' имеет дин. баланс '+FloatToStr(ADODMUnit.CurrDrBalance)+
        '! Это ниже минимума!');
  end;
end;
...
begin
  ...
  begin
    //if MessageDlg
    ShowMessage('Будет добавлена сумма '+FloatToStr
      (DrIncomesADOT.FieldByName('Summa').AsFloat)+
      ' к динамическому балансу водителя с позывным '+
...
begin
  ...
  begin
    ...
    ShowMessage('Таблица опций в настройках не содержит опций!');
  end
  else
    ShowMessage('Таблица опций неактивна!');
  end
  else
    ShowMessage('Таблица данных не содержит заявок!');
  end
  else
    ShowMessage('Таблица данных неактивна!');

  except on E:Exception do
    ShowMessage('Ошибка активации панели опций заказа! '+E.Message);
  end;
  finally
    fl.Free;
  end;
...
begin
  try
    ...
    ShowMessage('Ошибка инструкции сохранения опций заказа! '+E.Message);
  end;
end;
...
begin
  ...
  begin
    ...
    ShowMessage('Таблица опций пуста!');
  end
  else
    ShowMessage('Таблица опций неактивна!');
end;
...
begin
   if MessageDlg('Зарезервировать призовую заявку?', mtConfirmation,
...
begin
   if MessageDlg('Создать призовую заявку из резерных?', mtConfirmation,
    [mbYes,mbNo],0) = mrYes then
   begin
...
begin
  if (DrIncomesADOT.FieldByName('Summa').AsFloat<>0) and
    ...
    begin
      ...
      //DataSet.FindField()
      Abort;
      ShowMessage('Нельзя менять запись кассы при ненулевой сумме и позывном!');
...
begin
  if not (ActiveCustADOQuery.FindField('vypolnyaetsya_voditelem')
    .AsInteger>0) then
    begin
      ShowMessage('Нет связки с объектом водителя ' +
        '(попробуйте заново ввести позывной)!');
    end
  else
    begin
