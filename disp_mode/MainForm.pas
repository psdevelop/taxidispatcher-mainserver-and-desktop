...
begin
  ...
  begin
    ShowMessage('Приложение уже запущено, если оно не отвечает выключите его через Диспетчер задач!');
    //Application.Destroy;
    //Close;
...
begin
  if IniFile.ReadString('BUSINESS_DATA','спрашивать_о_бэкапе_базы',
    'NO')='YES' then
    begin
    if MessageDlg('Создать резервную копию базы?',
      mtConfirmation,[mbYes,mbNo],0) = mrYes then
      begin
        if not ADOConnectDM.SendSQL('BACKUP DATABASE '+
          IniFile.ReadString('BUSINESS_DATA','бэкап_путь',
            ' [TDTwoRelease0408] TO  DISK = N''C:\TDBackUp\TDTwoRelease0408.bak'' WITH NOFORMAT, NOINIT,  NAME = N''TDTwoRelease0408-Полная База данных Резервное копирование'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10')) then
        begin
          ShowMessage('Неудачное создание резервной копии БД!');
        end;
      end;
    end;
end;
...
if BoldModelDM.bsh.Active then
   ...
   if BoldHandlesDM.blhCurrUser.List.Count>0 then
    ...
    if (BoldHandlesDM.blhCurrUser.CurrentElement as TPersonal).Rolj_dispetchera
      ...
      begin
        ...
        'BUSINESS_DATA',
        'не_выводить_информацию_по_абонентскому_номеру',
        'NO')='YES' then
          ADOConnectDM.ActiveCustADOQuery.SQL.LoadFromFile(
   ...
   if ADOConnectDM.MainADOConnection.Connected then
     ...
     if not BoldModelDM.SendSQL('UPDATE Objekt_vyborki_otchyotnosti SET Ispoljz_priz_3='+
       IntToStr(Round(BoldModelDM.GetNextCustNumber))) then
         ShowMessage('Неудачное присвоение стартового числа счетчику заявок!');
     BoldModelDM.BoldUpdateDBAction1.Execute;
     end;
  end
else
 begin
  ...
  if IniFile.ReadString(
        'BUSINESS_DATA',
        'без_выхода_при_щелчке_по_кнопке_подключения',
        'NO')='YES' then
  begin
...
begin
   if ReportDM.NDROpenDialog.Execute then
    begin
      ...
      ShowMessage('Преобразование завершено, выходной файл - '+ExtractFileDir(ReportDM.NDROpenDialog.FileName)+'\'+StringReplace(PChar(ExtractFileName(ReportDM.NDROpenDialog.FileName)),StrScan(PChar(ExtractFileName(ReportDM.NDROpenDialog.FileName)),'.'),'',[rfReplaceAll])+'.rtf');
    end;
end;
...
begin
  if IniFile.ReadString('BUSINESS_DATA','scan_phone_line','NO')='YES' then
   begin
     if MessageDlg('Включить режим пропуска всех сигналов от модемов?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
       begin
   ...
   begin
     if MessageDlg('Отключить режим пропуска всех сигналов от модемов?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
       begin
...
begin
 ...
 mail.Add('Логин оператора: '+ADOConnectDM.
 CurrUserADOQuery.FieldByName(
   'Login').AsString);

 ADOConnectDM.Last8HourseArhADOQuery.Active:=False;
 try
   mail.Add('За последние 8 часов архивных заявок - '+
     '.');
 except
   ShowMessage('Неудачный запрос количества архивных заявок!');
 end;

 ADOConnectDM.Last8HoursePresentADOQuery.Active:=False;
 try
   ADOConnectDM.Last8HoursePresentADOQuery.Active:=True;
   mail.Add('За последние 8 часов призовых заявок - '+
     IntToStr(ADOConnectDM.Last8HoursePresentADOQuery.
     FieldByName('presents_count').AsInteger)+'.');
 except
   ShowMessage('Неудачный запрос количества призовых заявок!');
 end;

 mail.Add('Водителей на линии - '+', detailed:');
    ...
    begin
     try
      ...
      IdSMTPControl.Connect();
        ...
        else ShowMessage('Нет соединения с сервером, возможно не подключён Интернет!');
      finally
...
begin
  if FictOrdDelDateForm.ShowModal=mrOk then
    if MessageDlg('Удалить фиктивные заявки по '+DateToStr(
       ...
       begin
         ...
         try
           ...
           ShowMessage('Перезагрузите все клиентские приложения!');
         except
           ShowMessage('Неудачное прохождение запроса, перезагрузите все клиентские приложения!');
         end;
       end;
end;
...
begin
  if IniFile.ReadString('BUSINESS_DATA','отображать_дополнительные_характеристики_заявки','NO')='YES' then
     begin
       IniFile.WriteString('BUSINESS_DATA','отображать_дополнительные_характеристики_заявки','NO');
       ReleaseStructForm.AdditDispPageControl.Visible:=False;
       FirstForm.N35.Checked:=False;
     end
   else
     begin
       IniFile.WriteString('BUSINESS_DATA','отображать_дополнительные_характеристики_заявки','YES');
   ...
   if not InternetGetConnectedState(@dwConnectionTypes, 0) then
    ...
    begin
      if not InternetAutodial(INTERNET_AUTODIAL_FORCE_ONLINE or
       ...
       begin
         ShowMessage('Неудачная активизация соединения с Итернет по умолчанию!');
       end
      else
        res:=True;
     end
...
procedure TFirstForm.DeactivateDDU;
 begin
   if DisconnectDialUp = true then
     ShowMessage('Завершение интернет-операций.')
   else
...
begin
 ...
 begin
  try
    ...
    ShowMessage('Восстановление заявок произведено, перезагрузите систему!');
  except
    ShowMessage('Ошибка выполнения запроса на восстановление заявок!');
  end;
 end
 else
...
begin
  if MessageDlg('Создать резервную копию базы?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
                    begin
                        if not ADOConnectDM.SendSQL('BACKUP DATABASE '+
                          IniFile.ReadString('BUSINESS_DATA','бэкап_путь',' [TDTwoRelease0408] TO  DISK = N''C:\TDBackUp\TDTwoRelease0408.bak'' WITH NOFORMAT, NOINIT,  NAME = N''TDTwoRelease0408-Полная База данных Резервное копирование'', SKIP, NOREWIND, NOUNLOAD,  STATS = 10')) then
                            begin
                              ShowMessage('Неудачное создание резервной копии БД!');
                            end;
                    end;
...
begin
  ...
  begin
    ...
    try
        ...
        (Element as TVoditelj).Pozyvnoi)
          ...
          begin
           if (BoldModelDM.GetLocalDrRemoteSet(
            ...
            (Element as TVoditelj).Pozyvnoi)))>
            IniFile.ReadInteger('удаленный_контроль',
            'величина_таймаута_отсутствия',0)
            ) or
            (IniFile.ReadString('удаленный_контроль',
            'использовать_таймаут_отсутствия','NO')<>'YES'))
            then
             Result:=10
           else
              Result:=5;
      ...
      ShowMessage('Неудачное прохождение '+
      'запроса параметров водителя!');
    end;
    end;
  end;
end;
...
begin
  ...
  cidInSrc:=True;
  if IniFile.ReadString('BUSINESS_DATA',
    'сканировать_номера_от_SIP','NO')='YES' then
  begin
